function [m] = data2niivol(values, blacklistFileName, roisFileName, niiOutFileName)

path(path, '/scratch/braindata/shared/toolboxes/NIFTI')
...
funpsy_interpvol_rainer(cfg);
nii = load_nii(cfg.filename);
sum(nii.img(find(nii.img < 0) ) );


nii=fixOriginator(cfg.filename);
